    ...
    if st.session_state.selected_points.empty:
        ...
        if st.button("Start Training"):
            if not X_cols or not y_cols:
                ...
                st.line_chart(history.history['root_mean_squared_error'])
                
                # Plot Pred vs True
                st.subheader("Prediction vs Ground Truth (Training Set)")
                y_pred = model.predict(X)
                fig_res, ax_res = plt.subplots()
                ax_res.scatter(y.flatten(), y_pred.flatten(), alpha=0.6)
                ax_res.plot([y.min(), y.max()], [y.min(), y.max()], 'r--')
    ...
    if st.session_state.model is None:
        ...
        if st.button("Run Prediction on Full Dataset"):
            ...
            try:
                ...
                # Plot Ground Truth
                ax.plot_trisurf(x_axis, y_axis, y_true, color='green', alpha=0.3, label='Ground Truth')
                # Plot Prediction
                ax.plot_trisurf(x_axis, y_axis, y_pred, color='blue', alpha=0.6, label='Prediction')
                
                ax.set_xlabel(x_col_name)
                ax.set_ylabel(y_col_name)
                ax.set_title("Ground Truth (Green) vs Prediction (Blue)")
                
                # Fake legend since plot_trisurf doesn't support labels well in legend
                ...
                st.session_state.last_pred_results = {
                    "X": x_axis, "Y": y_axis, "Z": y_pred, "labels": meta
                }
...
# ================= PAGE 4: Surface View =================
    ...
    if not st.session_state.last_pred_results:
        ...
        surf = ax.plot_trisurf(res["X"], res["Y"], res["Z"], cmap='viridis', edgecolor='none')
